...
/// Initialize the keyring backend. Call once at startup before any password operations.
pub fn init_keyring() {
    secret::keyring::set_global_service_name("mailkit");
}

/// Retrieve the password for an IMAP account.
///
/// Lookup order:
/// 1. Environment variable `MAILKIT_PASSWORD_{ACCOUNT_NAME}` (override for CI/Docker)
/// 2. `password` field in config (Secret: raw, command, or keyring)
/// 3. Default keyring entry under "mailkit" service with username as key (backward compat)
pub async fn get_password(account_name: &str, config: &AccountConfig) -> crate::Result<String> {
    ...
    Err(crate::MailkitError::Credential(format!(
...
/// Otherwise, stores under the default "mailkit" service with the username as key.
