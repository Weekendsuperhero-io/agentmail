...
> **RFC 5550** — PROPOSED STANDARD  
> Authors: D. Cridland, Ed., A. Melnikov, Ed., S. Maes, Ed.  
> Published: August 2009  
> Obsoletes RFC4550; Updates RFC4467, RFC4469  
...
- General common enhancements to Internet Mail, described in 5 and 4.

- "Forward without download" that describes exchanges between Lemonade clients and servers to allow submitting new email messages incorporating content that resides on locations external to the client, described in Section 8.

- Quick mailbox resynchronization, described in Section 5.1.

- Extensions to support more precise, and broader, notifications from the store in support of notifications and view filters, described in 5.4.1 and 5.5.

- Delivery-time filtering in support of typical mail management use cases, as described in Section 3.3.
...
+------------------------------+--------------------+--------------+ | Sieve extension | Reference | Requirements | +------------------------------+--------------------+--------------+ | ENOTIFY | [SIEVE-NOTIFY] | Section 6 | | IMAP4FLAGS | [SIEVE-IMAP4FLAGS] | Section 6 | | RELATIONAL | [SIEVE-RELATIONAL] | Section 6 | | VACATION | [SIEVE-VACATION] | Section 6 | | VARIABLES | [SIEVE-VARIABLES] | Section 6 | | comparator-i;unicode-casemap | [UNICODE-CASEMAP] | Section 6 | +------------------------------+--------------------+--------------+
...
message to be sent. (See [IMAP-URLAUTH] for details of the semantics and steps. The so-called "pawn-ticket" authorization mechanism uses a URI that contains its own authorization credentials.)
...
pieces of the message to be sent. (See [SMTP-BURL] for details of the semantics and steps. The so-called "pawn-ticket" authorization mechanism uses a URI which contains its own authorization credentials.). I: {to S} Returns the requested body parts.
...
assembly succeeds, it acknowledges acceptance of the message by sending 250 response to the last BDAT command:
...
- Presence of proxy servers

- Presence of servers as intermediaries

- In general, the deployment models considered by OMA MEM that are not conventional IETF deployment models

- Measures to address a perceived need to traverse firewalls and mobile network intermediaries
