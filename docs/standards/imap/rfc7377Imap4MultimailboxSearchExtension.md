...
> **RFC 7377** — PROPOSED STANDARD  
> Authors: B. Leiba, A. Melnikov  
> Published: October 2014  
> Obsoletes RFC6237; Updates RFC4466  
...
- A single command limits the number of round trips needed to search a set of mailboxes.
...
- The CONTEXT return option (Section 4.2 of [RFC5267]) can be used with an ESEARCH command.

- If the UPDATE return option is used (Section 4.3 of [RFC5267]), it MUST apply only to the currently selected mailbox. If UPDATE is used and there is no mailbox currently selected, the ESEARCH command MUST return a "BAD" result.
...
- Added Section 2.3 about Fuzzy Search.
