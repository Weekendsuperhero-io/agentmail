...
> **RFC 7162** — PROPOSED STANDARD  
> Authors: A. Melnikov, D. Cridland  
> Published: May 2014  
> Obsoletes RFC4551, RFC5162; Updates RFC2683  
...
- a STORE command with the UNCHANGEDSINCE modifier, or

- an ENABLE command containing "CONDSTORE" as one of the parameters. (This option only applies when the client is communicating with a server that also implements the ENABLE extension [RFC5161].)
...
- the optional set of known UIDs, and
...
- Errata ID 3401 ("several typos in UNCHANGEDSINCE spelling")
- Errata ID 3506 ("invalid ABNF for the MODIFIED response code")
- Errata ID 3509 ("correction to an example")
...
## B. Changes since RFC 5162

Changed mod-sequences to be unsigned 63-bit values (instead of unsigned 64-bit values).

Addressed the following errata, as posted on <http://www.rfc-editor.org>:

- Errata ID 1365 ("clarified that QRESYNC is only enabled when ENABLED QRESYNC is returned")
- Errata ID 1807 ("unsolicited FETCH responses must include UID fetch response item")
- Errata ID 1808 ("HIGHESTMODSEQ response code must not be returned for CLOSE")
- Errata ID 1809 ("clarify how updated mailbox mod-sequence is calculated")
- Errata ID 1810 ("server must send untagged events to client in a way that client doesn't lose any changes in case of connectivity loss")
- Errata ID 3322 ("VANISHED responses must not reference non- existing UIDs")
