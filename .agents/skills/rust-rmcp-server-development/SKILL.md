...
description: Build, scaffold, update, debug, or review Rust Model Context Protocol servers, clients, and gateways using the official rmcp SDK. Use when the user mentions rmcp, Rust MCP servers or clients, MCP gateways or proxies in Rust, MCP tools in Rust, stdio or Streamable HTTP transports, ServerHandler, ClientHandler, tool_router, prompts, resources, sampling or elicitation handlers, spawning MCP servers from Rust, schemars or JSON Schema compatibility, structured tool output, MCP OAuth in Rust, Inspector testing, or replacing the outdated rust-mcp-server-generator skill.
license: Apache-2.0
---

# Rust RMCP Server Development

Use this skill to create or modernize Rust MCP servers, clients, and gateways with the official `rmcp` SDK.
...
Read `references/mcp-runtime-utilities.md` when designing capability negotiation, client/server/gateway roles, list change notifications, resource subscriptions, cancellation, ping, progress, tasks, timeouts, authorization, logging, completion, pagination, or Streamable HTTP security.
...
Read `references/gateway-patterns.md` when one process must be both MCP server and client: aggregating, proxying, filtering, or augmenting upstream servers.
...
- Role: server (default), client, or gateway.
...
- Clients advertise only implemented capabilities; gateways isolate downstream sessions and never forward downstream tokens upstream.
