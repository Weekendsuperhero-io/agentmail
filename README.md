...
One binary: `mailkit serve` starts the MCP stdio server, all other subcommands are a direct CLI.
...
Output binary: `target/release/mailkit`

## Configuration

mailkit reads its config from a single TOML file:

| Location | Path |
| --- | --- |
| Default | `~/.config/mailkit/config.toml` |
| Override | Set the `MAILKIT_CONFIG` environment variable to any path |

On macOS the default expands to `~/Library/Application Support/mailkit/config.toml` if `dirs::config_dir()` returns `Library/Application Support`, but `~/.config/mailkit/config.toml` is more conventional and works fine — just pick one.
...
mailkit uses [secret-lib](https://crates.io/crates/secret-lib) for flexible credential management. The `password` field supports three sources:
...
Stores and retrieves from the system credential store (macOS Keychain, Windows Credential Manager, Linux Secret Service). The value is the keyring entry key; the service name is `"mailkit"`. Store a password with:

```bash
mailkit set-password --account gmail
```
...
When connecting, mailkit tries these sources in order and uses the first one found:

1. `MAILKIT_PASSWORD_<ACCOUNT>` environment variable (override for CI/Docker)
2. `password` field in config (command, keyring, or raw)
3. Default keyring lookup under `"mailkit"` service with username as key (backward compat for `set-password` users with no `password` field)
...
```bash
export MAILKIT_PASSWORD_GMAIL="app-specific-password"
export MAILKIT_PASSWORD_WORK="your-password"
```

The variable name is `MAILKIT_PASSWORD_` followed by the account name uppercased, with dashes and spaces replaced by underscores.

### Testing your setup

```bash
# 1. Check that the account appears in the config
mailkit list-accounts

# 2. Test IMAP connectivity and authentication
mailkit check-connection --account gmail

# 3. List mailboxes to confirm full access
mailkit list-mailboxes --account gmail
```
...
```bash
mailkit configure gmail              # interactive account setup
mailkit configure                    # interactive setup (custom provider)
mailkit list-accounts
mailkit list-mailboxes --account gmail
mailkit create-mailbox --account gmail --name "Archive/2024"
mailkit check-connection --account gmail
mailkit list-capabilities --account gmail
mailkit set-password --account gmail
mailkit get-messages --account gmail --mailbox INBOX --limit 10
mailkit get-messages-by-uid --account gmail --uids 123 456
mailkit group-by-sender --account gmail --limit 20
mailkit find-attachments --account gmail
mailkit download-attachments --account gmail --uid 123 --output-dir ./downloads
mailkit list-flags --account gmail
```

Full subcommand list: `mailkit --help`
...
```json
{
  "mcpServers": {
    "mailkit": {
      ...
      "args": ["serve"],
      "env": {
        "MAILKIT_PASSWORD_GMAIL": "your-app-password"
      }
    }
  }
}
...
```
mailkit (binary crate: mailkit-mcp)
...
1. Run `mailkit check-connection --account <name>` to test connectivity.
2. Verify your password: `mailkit set-password --account <name>` to re-store it.
