...
// UIDFETCH (RFC 9586, IMAP UIDONLY extension): identical to FETCH, but the
// leading number is the message UID rather than a sequence number, and the
// UID message-data item may be omitted (it is redundant). Surface it as a
// normal `Response::Fetch` with a synthesized `Uid` attribute so existing
// FETCH consumers work unchanged and `.uid()` resolves.
//   [agentmail local patch — see docs/patches/]
